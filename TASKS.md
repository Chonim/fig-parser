...
## 다음에 할 것 (2026-09-07 기준, 우선순위 순)

정답지 비교로 다섯 건을 고친 뒤 남은 것. 번호는 GitHub 이슈 번호와 같다.

1. **matsq 정답지가 없다.** 오늘 고친 다섯 건 중 스트로크(레이아웃)와 그라디언트는
   전역 변경인데 kyowon 12프레임으로만 검증됐다. 이 문서의 규칙대로라면 **검증된 게
   아니다**. matsq 페이지를 한 장 내보내 `pnpm refs:slice ~/…png samples/matsq.fig`만
   하면 풀린다. 다른 무엇보다 이게 먼저다.
2. **`scroll` 그룹을 우리만 그린다.** ARCHIVE(복습 표) `2097:1378`, 4×531.
   두 자식 `2097:1379` `2097:1380`은 CENTER 4px 스트로크에 유효한 `strokeGeometry`
   (535×4, 255×4)를 갖고 `visible: true` `opacity: 1`, 가리는 것도 없다. 그런데 Figma
   내보내기에는 그 자리에 배경색만 있다. 약 2,000px. 지금까지 이런 "데이터는 그리라는데
   Figma는 안 그림"이 세 번 나왔고 셋 다 진짜 규칙이었다.
3. **Deco 코인 crop.** 23×23 코인 9장이 `w 0.6028 h 0.6152` crop을 갖는다. crop을 적용한
   뒤 이 프레임만 3,825px 나빠졌고 그 값이 코인 9장 넓이와 거의 같다 — 안티에일리어싱이
   아니라 자리가 틀렸다는 뜻. 반대 해석은 합계가 훨씬 나빠져서 채택하지 않았다.
   작은 정사각 박스 + 비정사각 원본이라는 조합만 따로 볼 것.
4. **boolean 칠 규칙의 근거.** "스트로크를 가진 boolean은 칠하지 않는다"는 측정으로
   고른 규칙이고 **왜 스트로크가 기준이어야 하는지는 모른다**. 두 무리를 실제로 가르는
   필드가 따로 있을 수 있다. matsq 정답지가 생기면 표본이 늘어 다시 볼 수 있다.
5. **3-1 constraints → CSS.** 참조가 생겨 보류 사유가 사라졌다. 아직 안 했다.
6. **3-3 `strokeAlign: CENTER` 111개.** 참조는 있지만 1x라 0.5px을 잴 수 없다.
   2x 내보내기가 있어야 판정된다.

이슈: https://github.com/Chonim/fig-parser/issues (번호가 위 항목 번호다)

바닥은 글자 렌더링이다. 남은 오차의 대부분이 글자 가장자리이고, Chrome과 Figma의
래스터라이즈 차이라 `get_html`(기하 기준선)에서 쫓을 값어치가 낮다.
...
- **정답지가 생겼다 (2026-09-06)** — kyowon-full 12프레임 전부. 페이지 한 장 내보내기를
  `pnpm refs:slice`로 잘랐다. 이걸로 **constraints → CSS(3-1)**과 **strokeAlign
  CENTER(3-3)**의 보류 사유가 사라졌다 — 이제 움직인 결과를 판정할 수 있다.
  첫 비교 결과와 거기서 나온 항목은 아래 "첫 정답지 비교"에 있다
...
- **`strokeAlign: CENTER`** — CSS에 반쪽 걸친 테두리가 없어 INSIDE로 근사.
  **"1개뿐"이라고 적었던 건 틀렸다.** 다시 세니 kyowon 1 + matsq 110 = **111개** <!-- fig:centred.strokes=111 -->이고,
  ...
  0.5px이다. 참조는 이제 있지만 **1x로 받은 것이라 0.5px은 잴 수 없다** — 이걸 판정하려면
  2x 내보내기가 필요하다. 재현하려면 박스를 weight만큼 키우고 절반만큼 당겨야 해서 흐름
  배치 자식의 위치까지 움직인다. 다만 스트로크를 `outline`/inset `box-shadow`로 옮긴 뒤로는
  테두리가 레이아웃에 참여하지 않으므로, 그때보다는 훨씬 작은 변경이 됐다
...
라벨을 두 번 세어, 멀쩡한 상자 5개를 초과로 불렀다. 26 → 28의 내역은
거짓 양성 5개(Button 3, Label 2)가 빠지고, 파생 좌표로 드러난 실제 초과 7개(TitleWrap)가
들어온 것이다.
...
```
경로 길이        1개 1781 · 2개 490 · 3개 65
첫 guid가        마스터 루트 354
                 마스터 서브트리의 자손 1811 (깊이 1:1311 2:303 3:169 4:17 5:17)
                 트리에 아예 없는 guid 165   <- 삭제된 노드의 잔재, 매칭 실패는 무시
필드             size 1604 · strokeGeometry 1013 · transform 640 · fillGeometry 336
                 derivedTextData 183 · minSize 12
```

**한 마스터 안에서는 깊이와 무관하게 guid 하나다.** 깊이 5짜리도 경로가 1개다.
경로가 2개 이상이면 **중첩 인스턴스를 타고 넘어간 것**: `[a, b]`는 이 마스터 안의
인스턴스 `a`, 그리고 `a`의 마스터 안의 노드 `b`를 뜻한다.

### 마지막 guid로 키를 잡으면 안 된다

한 인스턴스 안에서 **마지막 guid 기준 126건 충돌, 전체 경로 기준 0건**.
공유 마스터 안의 같은 노드에 서로 다른 체인으로 두 번 닿기 때문이다.
이전 시도가 깨진 이유가 이것이고, `components.test.mjs`가 두 수치를 다 고정한다.

다만 **이 파일에서는 두 키가 같은 결과를 낸다** — 충돌하는 항목들이 같은 값을 담고 있어
전체 경로로 잡든 마지막 guid로 잡든 IR이 노드 하나도 다르지 않다(전수 대조 0건).
그래서 이 가드는 예방적이고, 여기 있는 샘플로는 파손을 재현할 수 없다. 다른 파일에서
값이 갈리는 순간 조용히 틀리기 시작하므로 전체 경로를 유지한다.

### 읽지 않기로 한 것

- **`derivedTextData` 183건** — 새 정보가 없다. `size`를 같이 든 158건은 `layoutSize`가
  그 `size`와 완전히 같고, `size` 없는 25건은 마스터 노드 크기와 같다. `truncationStartIndex`는
  183건 전부 `-1`이라 Figma가 자른 텍스트도 없다. 읽어도 수치가 하나도 안 바뀐다.
- **`fillGeometry` 336건** — 적용해도 109프레임 렌더가 한 줄도 안 바뀐다.

둘 다 `components.test.mjs`가 고정한다. 다른 파일에서 값이 갈리면 그때 단언이 깨진다.
...
`translate(7 10)`으로 바뀌어 글자 중심보다 아래로 내려갔다. 4.67 = 7 × 16/24 —
바깥 항목의 좌표는 **바깥 마스터 공간**에 있고, 그걸 16 공간의 중첩 인스턴스에 그대로
꽂으면 축척이 어긋난다. 안쪽 인스턴스가 자기에 대해 들고 있는 값이 이미 축척된 값이다.

그래서 555건을 안쪽 항목과 전수 대조했다:

```
안쪽 항목과 완전히 동일        188
안쪽이 상위집합(공통 필드 일치) 278   <- 합쳐서 466건은 읽어도 달라지는 게 없다
값이 어긋남                     76
안쪽에 대응 항목이 아예 없음     13   <- 이 89건만 새 정보
```

새 정보 89건을 쓰려면 내려보내기 전에 **바깥 인스턴스/바깥 마스터 크기 비율로 좌표를
환산**해야 한다. 그 환산이 맞는지는 참조 이미지 없이는 판정할 수 없다 — 13프레임 224줄이
움직이는 변경이라 눈대중으로 넣을 것이 아니다.
...
`refs/<샘플명>/<frameId>.png` (Figma 2x 내보내기, id의 `:`는 `-`로).
없으면 skip, `CI=1`이면 exit 1. 참조 이미지는 gitignore — 샘플과 같이 사용자가 넣는다.

### 하네스가 믿을 만한지 먼저 증명한다

참조 없이도 두 가지를 확인할 수 있고, **하네스를 고칠 때마다 다시 한다**.

1. **자기 참조는 0이어야 한다.** 지금 렌더를 그대로 참조로 넣고 돌린다.
   0이 아니면 스크린샷이 재현되지 않는 것이고, 그 상태의 diff 수치는 전부 무의미하다.

2. **한 곳을 흔들면 그 프레임만 뜬다.** `boxRules`에서 특정 이름의 노드만 `left`를
   1px 옮기고 돌렸을 때:

```
온라인학습_My Page      58840   1.064  752×732 at 62,162
온라인학습_Login         4957   0.090  430×358 at 506,371
온라인학습_ARCHIVE        319   0.006  178×25 at 62,175
온라인학습_main             0   0.000  identical
```

그 노드가 없는 `main`은 0으로 남고, bbox가 실제로 움직인 자리를 가리킨다.
이 두 확인 없이 diff 수치를 근거로 쓰지 않는다.

### 이미 걸린 함정

스크린샷이 **웹폰트 도착 전에** 찍히고 있었다. 폰트 링크를 통째로 지워도 diff가 0이라
드러났다 — 양쪽 다 폴백으로 그리고 있었던 것. `--virtual-time-budget=8000`을 주면
같은 비교가 4121px 차이가 난다. 이걸 모르고 참조와 대조했다면 **모든 텍스트 픽셀이
거짓 차이**로 나왔을 것이다. 그래서 실행마다 웹폰트가 실제로 적용됐는지 한 번 재고,
안 됐으면 표 아래에 그렇게 적는다.

### 첫 정답지 비교 (2026-09-06)

12프레임 합계 **719,774px → 207,712px**. 표는 `pnpm diff`가 그대로 낸다.

내보내기 파일은 `samples/kyowon-full.fig`와 **바이트까지 같다** (sha256 a78b043f…).
디자인이 나중에 바뀐 게 아니라는 뜻이라, 차이는 전부 우리 것이다.

| 프레임 | 처음 | 지금 |
| --- | ---: | ---: |
| My Item | 11.304% | **1.520%** |
| ARCHIVE (책장) | 7.343% | **1.881%** |
| My Page | 7.837% | **0.999%** |
| ARCHIVE (복습 표) | 4.932% | **2.487%** |
| LEARNING QUEST | 4.188% | **2.122%** |
| 계정정보 | 4.699% | **0.671%** |
| Friend's Room | 4.366% | **1.168%** |
| main | 2.607% | **1.790%** |
| My Room | 2.008% | **0.902%** |
| Deco | 1.354% | **0.789%** |
| LEARNING QUEST (빈 상태) | 1.283% | **0.581%** |
| Login | 0.146% | **0.114%** |

**Login 0.146%가 이 비교의 기준선이다.** 한 화면이 통째로 맞고 남은 차이가 글자
안티에일리어싱뿐이라는 뜻이라, 나머지 프레임의 수치는 "렌더가 전반적으로 어긋난다"가
아니라 "이 프레임의 이것 하나가 틀렸다"로 읽어야 한다.

**고친 것 — 이미지 crop.** Figma의 "Crop"은 `imageScaleMode: STRETCH` + paint transform이고,
transform이 내용 전부다. 모드만 읽고 행렬을 버려서 잘라 놓은 사진이 원본 폭으로 다시
늘어나 있었다. 두 샘플의 STRETCH paint는 **전부** 비항등 transform을 갖는다 —
모드만으로는 아무것도 알 수 없다. `<img>`에는 `object-view-box`(Chrome 전용, 이 렌더는
Chrome에서 검증한다), background에는 size/position으로 낸다. 회전이 섞인 crop은 사각형이
아니라서 census가 그렇게 보고한다 — 두 샘플에는 0건.

**고친 것 — 스트로크를 가진 boolean은 칠을 하지 않는다.** My Item 패널(`2157:1899`)과
선택된 LEARNING QUEST 탭(`2111:98`). Figma 내보내기는 둘 다 테두리만 그리고 안은 배경이
그대로 비치는데, 파일은 둘 다 불투명 그라디언트와 도형 전체를 덮는 `fillGeometry`를 갖는다.
읽을 수 있는 필드로는 칠이 되는 boolean과 구분되지 않는다 — `visible: true`, `opacity: 1`,
불투명 스톱, 단일 contour(NONZERO), 마스크 없음, 순서도 맞고, `nodeChanges`에 중복 guid나
phase도 없다. 이 파일에서 두 무리를 가르는 필드는 스트로크 하나뿐이고, 그걸로 자르면
12프레임이 658,802 → 540,001px가 되며 나빠지는 프레임이 없다. 모든 boolean의 칠을 빼는 쪽은
더 나쁘다(560,940 — main의 채워진 union이 비어 버린다). **왜 스트로크가 기준이어야 하는지는
밝히지 못했다.** 측정으로 고른 규칙이고, 그 사실을 코드 주석에 그대로 적어 뒀다. IR은 여전히
paint를 보고한다 — 그리지 않기로 하는 건 렌더 쪽이다.

같은 패턴이 프레임 간에도 맞는다: Deco의 LEARNING QUEST 탭은 평범한 사각형이라 초록으로
칠해지고, LEARNING QUEST 프레임의 같은 탭은 boolean + 금색 테두리라 속이 비친다.

**고친 것 — 열린 경로에는 안팎이 없다.** `LINE`은 면적이 없어서 `strokeAlign`의
INSIDE/OUTSIDE가 가리킬 곳이 없고, Figma는 그런 스트로크를 아예 그리지 않는다. 그런데
파일에는 윤곽선을 만들어 넣어 둔다 — ARCHIVE 표의 행 구분선 11개가 각각 1244×1
`strokeGeometry`를 갖고 있어서, 그걸 칠하는 바람에 Figma 내보내기에는 없는 줄이 표에
그어졌다. 파일도 두 무리를 구분해 둔다: OUTSIDE 라인만 빈 `fillGeometry`를 달고 있고
CENTER 라인은 없다. 두 샘플에 INSIDE는 없고 OUTSIDE만 있다. 노드는 잉크만 빼고 IR에
그대로 남긴다 — 디자인에 있는 요소고, 빼 버리면 census가 15개를 설명하지 못한다.
ARCHIVE(복습 표) 4.465% → 3.477%, LEARNING QUEST 3.853% → 3.674%.

**고친 것 — 스트로크는 안에 있는 것을 밀지 않는다.** CSS `border`는 box-sizing과 무관하게
절대 배치된 모든 자손의 containing block을 자기 두께만큼 안으로 민다. Figma 스트로크는
아무것도 밀지 않는다. 책장 패널의 1px 스트로크 때문에 그 안의 책 표지 21장이 전부 Figma
내보내기보다 1px씩 아래·오른쪽에 찍혔다. 이런 패널이 어느 화면에나 있어서 **12프레임 전부**가
영향을 받았고, 이게 지금까지 나온 것 중 가장 큰 단일 원인이다: 523,871 → 213,864px.

`outline`(OUTSIDE)과 inset `box-shadow`(그 외)는 같은 테두리를 그리면서 레이아웃에
참여하지 않고, 둘 다 border-radius를 따라간다. 변 두께가 제각각인 경우와 그라디언트
스트로크는 여전히 진짜 `border`가 필요한데, 두 샘플에서 그것들은 전부 잎 노드다.

**고친 것 — 그라디언트는 각도가 아니라 핸들이다.** Figma는 그라디언트를 "램프가
(0,0)→(1,0)으로 흐르는 공간을 도형의 단위 정사각형으로 보내는 affine"으로 저장한다.
그 행렬을 **뒤집어서** 읽고 있었고, 게다가 각도만 뽑아 쓰고 있었다. CSS
`linear-gradient`는 램프를 언제나 박스 전체에 걸쳐 흘리지만 Figma 핸들은 아무 데나,
아무 길이나 될 수 있다 — 책장 선반의 램프는 선반 자체 폭의 약 12,000배라서 Figma는
한 가지 색으로 평평하게 칠하는데, 각도만 읽은 쪽은 9px 안에 밝음→어두움 전체를
그렸다. 이제 핸들을 CSS가 쓸 램프 선 위에 투영하고 각 스톱을 실제 떨어지는 자리에
놓는다. 0~100% 밖으로 나가는 값이 나오는데 CSS가 허용한다. SVG 쪽도 같은 행렬을
뒤집지 않고 그대로 쓴다. ARCHIVE(책장) 2.239% → 1.881%, 합계 213,864 → 207,712px.

**남은 것 — Deco 코인 (1.354% → 0.789%, 그래도 crop 적용 전후로 이 프레임만 나빠졌던 자리).** 23×23 코인 9개가 `w 0.6028 h 0.6152`
crop을 갖는다. crop을 적용하자 이 프레임만 3,825px 나빠졌고, 그 값이 코인 9개 넓이와
거의 같아서 안티에일리어싱이 아니라 자리 자체가 틀렸다는 뜻이다. 반대 해석(축소해서
여백을 두는 쪽)도 재 봤다 — Deco는 1.434%로 조금 낫지만 My Page가 5.405% → 11.554%로
무너지고 합계가 훨씬 나빠진다. 그래서 지금 해석을 유지하고, 작은 정사각 박스 + 비정사각
원본이라는 이 조합만 따로 본다
