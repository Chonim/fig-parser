...
### 행 그룹핑 — 완료
`layout.rows`가 자식을 세로 겹침으로 묶어 위→아래, 왼→오른쪽 순으로 낸다.
ARCHIVE 표에서 12개 밴드가 나오고 각 행이 `날짜 | 교재명 | 날짜 | 상태 | 버튼`으로 맞는다.

**힌트일 뿐 트리를 바꾸지 않는다.** 자식 순서를 재정렬하면 페인트 순서가 바뀌므로,
각 행은 그 노드 자신의 `children`에 대한 인덱스를 공백으로 이은 문자열이다
(id 배열로 냈더니 pretty-print에서 2386B, 인덱스 문자열로 520B).

주의: 컨테이너의 `layout`은 입양이 일어나기 **전에** 계산된다(입양은 부모 단계에서 일어남).
그래서 `nestByContainment`가 호스트의 힌트만 다시 계산한다. mode는 `absolute`로 고정 —
디자이너가 손으로 배치한 내용물을 flex라고 부르는 건 추측이다.
