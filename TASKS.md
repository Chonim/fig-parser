...
12프레임 합계 **719,774px → 658,802px**. 표는 `pnpm diff`가 그대로 낸다.

| 프레임 | 처음 | 지금 |
| --- | ---: | ---: |
| My Item | 11.304% | 11.041% |
| ARCHIVE (책장) | 7.343% | 7.343% |
| My Page | 7.837% | **5.405%** |
| ARCHIVE (복습 표) | 4.932% | 4.932% |
| LEARNING QUEST | 4.188% | 4.188% |
| 계정정보 | 4.699% | **3.755%** |
| Friend's Room | 4.366% | **3.574%** |
| main | 2.607% | 2.616% |
| My Room | 2.008% | **1.744%** |
| Deco | 1.354% | 1.630% |
| LEARNING QUEST (빈 상태) | 1.283% | 1.283% |
| Login | 0.146% | 0.146% |
...
**남은 것 1 — My Item 패널 (11.041%, 이 프레임 오차의 거의 전부).**
`2157:1899` Union의 몸통을 우리는 초록으로 칠하고 Figma 내보내기는 비워 둔다.
패널 몸통 넓이에서 탭·썸네일이 가리는 부분을 뺀 값이 오차 픽셀 수와 거의 일치하니
다른 원인이 섞여 있지 않다. 그런데 데이터는 전부 칠하라고 한다: 노드 `visible: true`
`opacity: 1`, `fillPaints[0]`은 불투명 `GRADIENT_LINEAR`, `fillGeometry`는 패널 전체를
덮는 단일 contour(면적 279189, NONZERO), 두 피연산자(`2157:1858` SOLID 초록,
`2157:1820` 그라디언트 = 혀 부분)도 둘 다 불투명 초록, 마스크 없음, 그리는 순서도
방 뒤가 아니라 앞(`";"` > `":"`)이다. `nodeChanges`에 같은 guid의 중복도 phase도 없어
낡은 값을 읽는 것도 아니다. **읽을 수 있는 모든 필드가 초록이라고 말한다.**
그래서 지금 서 있는 자리는 "내보내기가 샘플보다 나중 상태일 가능성"이고, 확인 없이
허용 오차로 덮지 않는다. 확인 방법은 하나뿐이다 — 사용자에게 이 내보내기가
`samples/kyowon-full.fig`와 같은 시점인지 묻는 것

**남은 것 2 — Deco 코인 (1.354% → 1.630%).** 23×23 코인 9개가 `w 0.6028 h 0.6152`
