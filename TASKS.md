...
- **`strokeAlign: CENTER`** — CSS에 반쪽 걸친 테두리가 없어 INSIDE로 근사.
  **"1개뿐"이라고 적었던 건 틀렸다.** 다시 세니 kyowon 1 + matsq 110 = **111개** <!-- fig:centred.strokes=111 -->이고,
  ...
  0.5px이고, 2x 렌더에서 1 디바이스 픽셀이다. 그 1픽셀이 맞게 나왔는지는 참조 이미지로만
  판정할 수 있는데 아직 없다. 재현하려면 박스를 weight만큼 키우고 절반만큼 당겨야 해서
  흐름 배치 자식의 위치까지 움직인다 — 맞는지 볼 수 없는 상태에서 건드릴 변경이 아니다.
  참조가 도착하면 `pnpm diff`의 오차 상위에 이게 뜨는지부터 보고 결정할 것
