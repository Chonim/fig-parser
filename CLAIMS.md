...
| C11 | 도구는 7개다 | 1-2 | 검증(docs.test: `fig:tools.count`, 정의 배열에서 측정) |
...
| # | 주장 | 담당 | 상태 |
| --- | --- | --- | --- |
| D1 | 문서가 이름 붙인 21개 프레임 id가 샘플에 존재한다 | 3-2 | 실측 불가 표시 — 아래 참조 |
| D2 | 문서가 이름 붙인 7개 노드 id가 샘플에 존재한다 | 3-2 | 실측 불가 표시 — 아래 참조 |
| D3 | `REFS.md`의 파일명 규약 `2063:280 → 2063-280.png` | 2-2 | 검증(diff.mjs의 `refPath`가 그 규약이고, docs.test가 `refs/` 부재를 문서와 대조) |

**D1·D2가 "실측 불가"인 이유**: 0-1에서 33개를 전부 대조해 21 프레임 · 7 노드가 실재하고
나머지 5개는 깊이 히스토그램 오탐임을 확인했다. 다만 상시 검사로 만들려면 산문에서 `n:m`을
긁어야 하고, 그 오탐이 바로 긁기가 왜 안 되는지의 증거다. 문서가 예시로 드는 id는
`fig:` 마커가 붙은 수치들과 달리 실행 경로(worked pass, 도구 주장)에 들어간 것만 검사한다 —
`2063:280` `2063:289` `2063:301` `97:3081` `43:783` `2313:1353` `2067:2`가 그렇게 매 실행 쓰인다.

## E. 수치

문서에 두 자리 이상 숫자가 든 줄이 143개다. 산문에서 숫자를 긁으면 오탐이 나오므로
(깊이 히스토그램이 프레임 id로 읽히고, 고쳐진 결함의 서술이 현재 수치로 읽힌다)
**대조할 수치에는 `fig:` 마커를 붙인다** (`fig:` 뒤에 측정 키, `=`, 값). 지금 30개가 붙어 있고 `pnpm docs:check`가
