...
green run on a fresh clone proves nothing. Confirm the sample files exist first.
...
expanding instances against their master, and inferring layout. `HANDLED` declares what this
layer can express; `census.mjs` reads it rather than keeping its own copy, so the report cannot
drift from the code.

**mcp.mjs** keeps two different shapes of the same tree. Renderers want every bezier; a model
needs to know a 246×98 logo is present and can be exported. `forModel` strips path data and
`fit` cuts depth until the response fits a 30KB budget, replacing cut children with stubs that
name an id to pass back as `select`. Both `depth` and `includePaths` bypass the budget.
...
- **No rendering code a sample cannot check.** Radial gradients, stacked fills, constraints and
  wrap-grid inference are all deliberately unimplemented because neither sample contains them.
  `TASKS.md` records the evidence for each.
- **Census must balance.** `raw = IR + collapsed + folded + invisible`. A "nodes that vanished"
  row means something is being dropped silently — that is a bug, not a rounding artefact.
